...
export class UALJs extends UAL {
    ...
    setTimeout(() => {
      if (!!authenticators.autoLoginAuthenticator) {
        ...
        if (!this.renderConfig) {
          throw new Error(
            "Render Configuration is required when no auto login authenticator is provided"
          );
        }

        const { containerElement, buttonStyleOverride = false } = this
          .renderConfig as UALJsRenderConfig;
  ...
  private attemptSessionLogin(authenticators: Authenticator[]) {
    const sessionExpiration =
      localStorage.getItem(UALJs.SESSION_EXPIRATION_KEY) || null;
    if (sessionExpiration) {
      ...
      if (new Date(sessionExpiration) <= new Date()) {
        ...
        const accountName =
          localStorage.getItem(UALJs.SESSION_ACCOUNT_NAME_KEY) || undefined;
